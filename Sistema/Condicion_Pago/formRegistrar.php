...
<body>
    <form role="form" id="formRegistrarCondicion">
		<input type="hidden" name="NB" id="NB" value="<?php echo $NB;?>">
        <div class="row">  
            <div class="col-lg-6">            
                <div class="form-group">
                    <label>CONDICION:</label>
                    <select class="form-control" required name="NB" id="NB" onChange="localidad()">
                    	<option value="" disabled selected>Seleccione...</option>
<?php 
						$Conector=Conectar();	

						$vSQL="EXEC [dbo].[SP_CONDICION_INSERT] (VALOR,USUARIO)		Tipo de Valor: VARCHAR, NUMERIC";
                        ...
                        {		
                            ...
                            {				
								...
								$FG_ACTIVO		=utf8_encode(odbc_result($result,'FG_ACTIVO')
								$LOGIN_USEROL	=utf8_encode(odbc_result($result,'LOGIN_USEROL')
								$F_REG=utf8_encode(odbc_result($result,'F_REG'));								
                                        
                                echo "<option value='$NB'>$LOGIN_USEROL</option>";
                            }
                        }
                        else
                        {	
    ...
    <script>
		...
		function localidad()
		{
			...
			$.ajax(
			{
				type: "POST",
				dataType:"html",
				url: "Sistema/Condicion_Pago/scriptLocalidad.php",
                data: Parametros,
				beforeSend: function()
				{
		...
		function guardar()
		{
			...
			if ($('#TLF').val().length!=12) {
				window.parent.MostrarMensaje("Rojo", "Disculpe, Ingrese un n&uacutemero de celular v&aacutealido.");
				$("#TLF").focus();
				return;	
			}
			...
			$.ajax(
			{
				type: "POST",
				dataType:"html",
				url: "Sistema/Condicion_Pago/scriptRegistrar.php",
				data: Parametros,
				beforeSend: function()
				{
					window.parent.Cargando(1);
					...
					else
					{
						...
						else
						{
                            ...
                            if(RESULTADO==1)
                            {
                                ...
                                window.parent.MostrarMensaje("Verde", "Registro realizado exit&oacutesamente!");
